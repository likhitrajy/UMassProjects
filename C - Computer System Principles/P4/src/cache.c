...
Cache *make_cache(int set_bits, int line_count, int block_bits) {
  ...
  int set_count1 = exp2(set_bits);
  int block_size1 = exp2(block_bits);
 
  cache->sets =  make_sets(set_count1, line_count, block_size1);
  cache->set_count = set_count1;
  cache->line_count = line_count;
  cache->block_size = block_size1;
