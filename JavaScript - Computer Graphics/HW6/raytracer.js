	...
	if(isect === null){
		...
		if ((isect.material.kr || isect.material.kt) && (depth < maxDepth)) {
			...
			if(isect.material.kr){
				let refl_ray = new Ray(isect.position, reflect(ray.direction().negate(), isect.normal)); 
				color.add((isect.material.kr.clone()).multiply(raytracing(refl_ray, depth+1)));
			}
			if(isect.material.kt){
				let refl_ray = new Ray(isect.position, refract(ray.direction().negate(), isect.normal, isect.material.ior))
				color.add((isect.material.kt.clone()).multiply(raytracing(refl_ray, depth+1)));
			} 

		} else if (isect.material.ka){
			color = ambientLight.clone().multiply(isect.material.ka);
...
function shading(ray, isect) {
	let color = new THREE.Color(0,0,0);
// ===YOUR CODE STARTS HERE===
	let kd = isect.material.kd == null ? new THREE.Color(0, 0, 0) : isect.material.kd;
	let ks = isect.material.ks == null ? new THREE.Color(0, 0, 0) : isect.material.ks;
	...
	for(let i=0; i<lights.length; i++) {
		...
		if (shadow_isect && shadow_isect.t < distToLight){continue; /* shadowed */}

		color.add(ls.intensity.clone().multiply(kd).multiplyScalar(Math.max(isect.normal.clone().dot(ls.direction), 0))).add(ls.intensity.clone().multiply(ks).multiplyScalar(Math.max(reflect(ls.direction, isect.normal).dot(ray.d.clone().negate()), 0)**isect.material.p));
	}
