...
def constructBayesNet(gameState: hunters.GameState):
    ...
    for p1 in positions:
        for p2 in positions:
            trueVal = manhattanDistance(p1, p2)
            obs_1 = abs(trueVal + MAX_NOISE)
            obs_2 = abs(trueVal - MAX_NOISE)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for v in eliminationOrder:
            currFactors, joinedFactor = joinFactorsByVariable(currFactors, v)

            if 2 <= len(joinedFactor.unconditionedVariables()):
                elimFactor = eliminate(joinedFactor, v)
                currFactors.append(elimFactor)

        qeFact = normalize(joinFactors(currFactors))
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for i in self.items():
            seq.append(i[0])
            w.append(float(i[1])/float(total))
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        newBel = DiscreteDistribution()
        positionDist = {}
        for pos in self.allPositions:
            if pos in positionDist:
                newPosDist = positionDist[pos]
            else:
                newPosDist = self.getPositionDistribution(gameState, pos)
                positionDist[pos] = newPosDist
            for key in newPosDist.keys():
                newBel[key] += newPosDist[key]*self.beliefs[pos]
        self.beliefs = newBel
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        pacPos = gameState.getPacmanPosition()
        jailPos = self.getJailPosition()
        weightedParts = DiscreteDistribution()
        weights = {}

        for p in self.particles:
            if p not in weights:
                weight = self.getObservationProb(observation, pacPos, p, jailPos)
                weights[p] = weight

            weightedParts[p] += weights[p]

        if weightedParts.total() == 0:
            self.initializeUniformly(gameState)
            return None

        newParts = [weightedParts.sample() for _ in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPart in self.particles:
            if oldPart not in newPosDists:
                newPosDist = self.getPositionDistribution(gameState, oldPart)
                newPosDists[oldPart] = newPosDist

            newParts.append(newPosDists[oldPart].sample())
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        prod = list(itertools.product(self.legalPositions, self.legalPositions))
        random.shuffle(prod)
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for p in self.particles:
            ...
            for ghost in range(self.numGhosts):
                ...
                if (ghost, p[ghost]) not in weights:
                    weight = self.getObservationProb(observation[ghost], pacPos, p[ghost], jailPos)
                    weights[(ghost, p[ghost])] = weight

                totalWeight *= weights[(ghost, p[ghost])]

            weightedParts[p] += totalWeight
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            if oldParticle not in newPosDists:
                newPosDists[oldParticle] = []
                for ghost in range(self.numGhosts):
                    PosDist = self.getPositionDistribution(gameState, oldParticle, ghost, self.ghostAgents[ghost])
                    newPosDists[oldParticle].append(PosDist)

            for ghost in range(self.numGhosts):
                newParticle[ghost] = newPosDists[oldParticle][ghost].sample()
